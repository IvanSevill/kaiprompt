...
function runnable(pending, t) {
  return {
    prio: pending.filter(isPriority).sort((a, b) => (a.createdAt ?? 0) - (b.createdAt ?? 0)),
    due: pending.filter((j) => !isPriority(j) && j.when && j.when <= t).sort((a, b) => a.when - b.when),
    seq: pending.filter((j) => !isPriority(j) && !j.when),
  };
}
