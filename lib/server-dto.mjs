...
function jobDTO(job, context = {}) {
  ...
  return {
    ...
    conversationId: conversationIdForJob(job, context),
    adapter: job.adapter,
...
export function stateDTO() {
  ...
  const jobs = queue.map((job) => jobDTO(job, { queue, sessions }));
...
export function targetsDTO({ openCodeRun } = {}) {
  ...
  for (const [target, raw] of Object.entries(sessions)) {
    ...
    for (const record of records) {
      const jobs = queue.filter((job) => (job.adapter === record.adapter && job.sessionId === record.sessionId)
  ...
  const summaries = groups.map(({ target, record, jobs }) => {
    ...
    return {
      conversationId: conversationIdForRecord(target, record, jobs),
  ...
  const duplicates = summaries.reduce((counts, summary) => {
    counts.set(summary.conversationId, (counts.get(summary.conversationId) ?? 0) + 1);
    return counts;
  }, new Map());
  ...
  return summaries
    .sort((a, b) => {
      const ak = `${a.conversationId}:${a.target ?? ''}:${a.adapter ?? ''}:${a.sessionId ?? ''}:${a.jobs.join(',')}`;
      const bk = `${b.conversationId}:${b.target ?? ''}:${b.adapter ?? ''}:${b.sessionId ?? ''}:${b.jobs.join(',')}`;
      return ak.localeCompare(bk);
    })
    .map((summary) => {
      let conversationId = summary.conversationId;
      if ((duplicates.get(conversationId) ?? 0) > 1) {
        const key = `${conversationId}:${summary.target ?? ''}:${summary.adapter ?? ''}:${summary.sessionId ?? ''}`;
        const ordinal = ordinals.get(key) ?? 0;
        ordinals.set(key, ordinal + 1);
        conversationId = conversationAlias(
          conversationId, summary.target, summary.adapter, summary.sessionId, ordinal,
        );
      }
      const terminal = ['done', 'error', 'missed'].includes(summary.status);
      return { ...summary, conversationId, hidden: terminal && hidden.has(conversationId) };
    })
    .sort((a, b) => b.updatedAt - a.updatedAt || a.conversationId.localeCompare(b.conversationId));
}

export function snapshotDTO(options = {}) {
  const state = stateDTO();
  return { state, conversations: targetsDTO(options) };
}

/** Hide terminal conversations without deleting any provider or Kaiprompt history. */
export function hideFinishedConversations(options = {}) {
  const summaries = targetsDTO(options);
  const finished = summaries.filter((summary) => ['done', 'error', 'missed'].includes(summary.status));
  const active = summaries.length - finished.length;
  const ids = finished.filter((summary) => !summary.hidden).map((summary) => summary.conversationId);
  const result = hideConversationIds(ids);
  return {
    ok: true,
    hidden: result.added,
    alreadyHidden: finished.length - ids.length,
    activeRetained: active,
    totalHidden: result.total,
  };
}
...
function withLiveEvents(base, jobs, streamJob) {
  const turns = base.turns.map((turn) => ({
    ...
    diffs: (turn.diffs ?? []).map((diff) => ({ ...diff, lines: [...(diff.lines ?? [])] })),
  }));
  const available = new Map();
  const availableDiffs = new Map();
  for (const turn of turns) for (const block of turn.blocks) {
    const key = blockKey(block);
    if (key) available.set(key, [...(available.get(key) ?? []), block]);
  }
  for (const turn of turns) for (const diff of turn.diffs) {
    if (diff.id) availableDiffs.set(diff.id, [...(availableDiffs.get(diff.id) ?? []), diff]);
  }
  ...
  const unmatched = [];
  const unmatchedDiffs = [];
  const knownDiffIds = new Set(turns.flatMap((turn) => turn.diffs.map((diff) => diff.id)).filter(Boolean));
  for (const event of events) {
    if (consumed.has(event.id)) continue;
    if (event.kind === 'diff' && event.diff?.id) {
      const match = availableDiffs.get(event.diff.id)?.shift();
      if (match) match.eventId = event.id;
      else if (!knownDiffIds.has(event.diff.id)) {
        knownDiffIds.add(event.diff.id);
        unmatchedDiffs.push({ ...event.diff, eventId: event.id });
      }
      continue;
    }
  ...
  if (unmatched.length || unmatchedDiffs.length) turns.push({
    role: 'assistant', at: new Date(unmatched[0]?.event.at ?? events.find((event) => event.kind === 'diff')?.at).toISOString(),
    toolResult: false, sidechain: false, live: true,
    blocks: unmatched.map((item) => item.block), diffs: unmatchedDiffs,
  });
...
export function chatDTO(ref, { openCodeRun } = {}) {
  const queue = loadQueue();
  const summary = targetsDTO({ openCodeRun }).find((candidate) => candidate.conversationId === ref);
  const resolvedRef = summary?.ref ?? ref;
  const direct = queue.find((job) => job.id === resolvedRef);
  const { resolved, chat } = loadConversation(resolvedRef, { openCodeRun, pendingExternal: true });
  ...
  const base = {
      conversationId: summary?.conversationId ?? conversationIdForRecord(target, null, jobs),
      sessionId,
      target,
...
function diffsFromTurn(turn) {
  const diffs = turn.blocks.flatMap((block) => {
    return block.type === 'tool_use' ? normalizeToolDiffs(block.name, block.input) : [];
  });
  return [...new Map(diffs.map((diff) => [diff.id, diff])).values()];
}
...
export function outputDTO(id) {
  ...
  return {
    ...jobDTO(job, { queue: loadQueue(), sessions: loadSessions() }),
