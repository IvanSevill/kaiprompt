...
function adapterChatDTO({ sessionId, target, jobs }) {
  const turns = [];
  ...
  return {
    ...
    first: turns[0]?.at ?? null, last: turns.at(-1)?.at ?? null, turns,
  };
}
...
export function chatDTO(ref) {
  const direct = loadQueue().find((job) => job.id === ref);
  const fallback = direct && ['opencode', 'codex'].includes(direct.adapter)
    ? { sessionId: direct.sessionId ?? `job:${direct.id}`, target: direct.target ?? null, jobs: [direct] }
    : null;
  ...
  if (!file) {
    if (external) {
      return adapterChatDTO({ sessionId, target, jobs });
    }
