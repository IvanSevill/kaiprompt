...
import { isQuotaExhausted, parseResetAt } from './quota.mjs';
import { CONTINUATION } from './prompt.mjs';
import { addJob } from './queue.mjs';
import { DATA, loadQueue, loadSessions, readJSON, writeJSON } from './store.mjs';
...
export const isQuotaError = (turn) =>
  Boolean(turn && turn.role === 'assistant' && turn.apiError && isQuotaExhausted(flatten(turn)));
