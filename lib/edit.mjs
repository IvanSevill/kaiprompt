...
export const EDITABLE = ['prompt', 'from', 'at', 'target', 'dir', 'perm', 'adapter', 'model'];
...
export function applyEdits(job, flags = {}) {
  ...
  if ('model' in flags) {
    const v = value(flags, 'model');
    if (CLEARS.includes(String(v).toLowerCase())) set('model', null);
    else if (!v) throw new Error('--model cannot be empty');
    else set('model', v);
  }
