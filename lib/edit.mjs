...
export const EDITABLE = ['prompt', 'from', 'at', 'target', 'dir', 'perm', 'engine', 'provider', 'model'];
...
export function applyEdits(job, flags = {}) {
  ...
  const selectionKeys = ['adapter', 'engine', 'provider', 'model'];
  if (selectionKeys.some((key) => key in flags)) {
    const engine = 'engine' in flags ? value(flags, 'engine') : ('adapter' in flags ? value(flags, 'adapter') : next.adapter);
    const provider = 'provider' in flags ? value(flags, 'provider') : next.provider;
    const rawModel = 'model' in flags ? value(flags, 'model') : next.model;
    const model = CLEARS.includes(String(rawModel).toLowerCase()) ? null : rawModel;
    const selection = normalizeSelection({ adapter: engine, provider, model }, { required: true });
    if (selection.adapter !== next.adapter) {
      set('adapter', selection.adapter);
      // Sessions are engine-specific. A new engine receives the original prompt, never a foreign continuation.
      for (const key of ['sessionId', 'continuation', 'pausedUntil', 'quotaRetries']) {
        if (next[key] != null && next[key] !== false && next[key] !== 0) set(key, null);
      }
    }
    if (selection.provider !== (next.provider ?? null)) set('provider', selection.provider);
    if (selection.model !== (next.model ?? null)) set('model', selection.model);
  }
...
export function editJob(id, flags = {}) {
  if (!id) throw new Error('usage: kaip edit <id> [--prompt …] [--at …] [--target …] [--dir …] [--perm …] [--engine …]');
