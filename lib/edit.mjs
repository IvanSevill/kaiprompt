...
export function editJob(id, flags = {}) {
  importProgramados();
  if (!id) throw new Error('usage: promptheus edit <id> [--prompt …] [--at …] [--target …] [--dir …] [--perm …] [--adapter …]');

  const job = loadQueue().find((j) => j.id === id);
  if (!job) throw new Error(`no job found with id "${id}" (see: promptheus list)`);
