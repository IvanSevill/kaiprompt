// Edit a job that hasn't run yet (including one scheduled from the chat with /programar).
//
...
import { ADAPTERS, importProgramados, loadQueue, patchJob, resolveDir } from './store.mjs';
import { parseWhen } from './time.mjs';

export const EDITABLE = ['prompt', 'at', 'target', 'dir', 'perm', 'adapter'];
...
/**
 * Edit job `id` in the queue and persist it. Imports the /programar inbox first,
 * so a launch scheduled from the chat can be edited before it ever runs.
 */
export function editJob(id, flags = {}) {
  importProgramados();
  if (!id) throw new Error('usage: program-prompt edit <id> [--prompt …] [--at …] [--target …] [--dir …] [--perm …] [--adapter …]');

  const job = loadQueue().find((j) => j.id === id);
  if (!job) throw new Error(`no job found with id "${id}" (see: program-prompt list)`);
  if (job.status !== 'pending') {
    throw new Error(`job "${id}" is ${job.status}: only pending jobs can be edited.\n`
      + `  its result: program-prompt out ${id}`);
  }
