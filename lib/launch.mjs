...
import {
  ADAPTERS, HOME, loadSessions, nowMs, outPath, patchJob, rememberSession,
} from './store.mjs';
...
export async function executeJob(job, { dryRun = false, onEvent } = {}) {
  ...
  const sid = job.sessionId || (key && loadSessions()[key]?.sessionId) || null;
  ...
  const res = await adapter.run({
    ...
    permMode: job.permMode || null, model: job.model || null, onEvent,
  });
  ...
  if (res.sessionId) {
    ...
    if (key) rememberSession(key, res.sessionId, job.adapter);
  }
  return res;
}
