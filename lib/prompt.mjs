...
export const CONTINUATION = [
  ...
  '',
  'Se acabó el cupo a mitad del trabajo y ahora se ha renovado. Esta conversación es la',
  'misma: ya tienes el contexto y lo que llevabas hecho. NO empieces de cero ni repitas lo',
  'que ya está hecho — revisa en qué punto estabas y sigue desde ahí.',
].join('\n');
...
export function jobPreview(job, n = 60) {
  ...
  catch { return isLinked(job) ? `⚠ ${path.basename(job.promptFile)} — ilegible` : ''; }

  const first = text.split('\n').find((l) => l.trim()) ?? '';
  const t = first.replace(/\s+/g, ' ').trim();
  const cut = t.length > n ? t.slice(0, n - 1) + '…' : t;
  return isLinked(job) ? `↪ ${cut}` : cut;      // ↪ = viene de un archivo
}
