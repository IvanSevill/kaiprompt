...
import { discoverOpenCodeModels, engineNames } from './engines.mjs';
...
export const STEPS = [
  ...
  { key: 'from', label: 'Prompt file', hint: 'optional --from path; when set, the file is read at launch instead of the prompt above' },
  ...
  { key: 'model', label: 'Model', hint: 'optional for Claude/Codex; required for OpenCode', },
...
export function wizardChoices(step, values) {
  ...
  if (step.key === 'model') {
    ...
    return discoverOpenCodeModels(values.provider).map((m) => m.model);
  }
  return step.choices || [];
}
