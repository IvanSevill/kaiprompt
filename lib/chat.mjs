...
export function resolveRef(ref) {
  ...
  if (saved?.sessionId) return { sessionId: saved.sessionId, target: ref, adapter: saved.adapter, jobs: jobsOf(saved.sessionId) };
...
export function renderChat(ref, { last = 20, full = false, raw = false } = {}) {
  ...
  if (!file) {
    if (jobs.some((job) => ['opencode', 'codex'].includes(job.adapter))) {
      const lines = [];
      for (const job of jobs) {
        let prompt = '';
        try { prompt = resolvePrompt(job); } catch { /* the job's own error is shown below */ }
        if (prompt) lines.push(c.accent('you'), ...wrap(prompt, Math.min(size().cols, 100) - 2));
        let output = '';
        try { output = fs.readFileSync(outPath(job.id), 'utf8').trim(); } catch { /* no result yet */ }
        if (output || job.error) lines.push('', c.ok(job.adapter), ...wrap(output || String(job.error), Math.min(size().cols, 100) - 2));
      }
      return [...box([
        c.muted('target  ') + (target || '—'),
        c.muted('session ') + sessionId,
        c.muted('engine  ') + (jobs[0]?.adapter || '—'),
      ], { title: 'chat', cols: Math.min(size().cols - 2, 78) }), '', ...lines].join('\n');
    }
