...
export function addJob({
  ...
  adapter = 'claude', engine = null, provider = null, model = null, session = null, cwd = process.cwd(),
  priority = false, continuation = false,
} = {}) {
  const promptFile = from ? linkPrompt(from) : null;
  const text = String(prompt ?? '').trim();
  if (model != null && !String(model).trim()) throw new Error('model cannot be empty');
  ...
  if (job.sessionId && job.target) {
    ...
    const record = { sessionId: job.sessionId, adapter: job.adapter, provider: job.provider, model: job.model, updatedAt: nowMs() };
    sessions[job.target] = { ...record, engines: { [job.adapter]: record } };
...
export function suggestTargets() {
  ...
  for (const [name, raw] of Object.entries(sessions)) {
    const s = raw?.engines ? Object.values(raw.engines).sort((a, b) => (b.updatedAt ?? 0) - (a.updatedAt ?? 0))[0] : raw;
    if (!s) continue;
    byName.set(name, {
...
export function jobDetails(job) {
  ...
  if (job.continuation) rows.push(['resume', 'continues the conversation; the prompt is not re-sent']);
