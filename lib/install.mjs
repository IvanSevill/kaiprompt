...
import { writeJSON } from '../src/storage/json.mjs';
...
export function defaultClaudeUsageRoot(root, env = process.env) {
  return path.resolve(env.CLAUDE_USAGE_ROOT || path.join(root, '..', 'claude-usage'));
}

export function opencodePluginBody(toolRoot) {
  const source = pathToFileURL(path.resolve(toolRoot, 'opencode', 'usage-metrics.mjs')).href;
  return `// Generated by Kaiprompt; implementation lives in claude-usage.\nexport { default } from ${JSON.stringify(source)};\n`;
}

/** Recognise marked loaders and the old generated loader only when it points inside Kaiprompt. */
export function isKaipromptPluginLoader(body) {
  if (typeof body !== 'string') return false;
  const normalized = body.replace(/\\/g, '/');
  return /^\/\/ Generated by Kaiprompt; implementation lives in claude-usage\.\r?\nexport \{ default \} from ["']file:\/\/\/.+\/opencode\/usage-metrics\.mjs["'];\r?\n$/.test(normalized)
    || /^export \{ default \} from ["']file:\/\/\/.+\/kaiprompt\/opencode\/usage-metrics\.mjs["'];\r?\n$/.test(normalized);
}
...
export function install({
  root, claudeDir = claudeHome(), opencodeDir = opencodeHome(), base = null,
  claudeUsageRoot = defaultClaudeUsageRoot(root),
} = {}) {
  const done = [];
  ...
  if (fs.existsSync(opencodePlugin)) {
    let existing = null;
    try { existing = fs.readFileSync(opencodePlugin, 'utf8'); } catch { /* report and preserve */ }
    const wanted = opencodePluginBody(claudeUsageRoot);
    if (existing === wanted) done.push('plugin   kaiprompt-usage.js already current');
    else if (isKaipromptPluginLoader(existing)) {
      fs.writeFileSync(opencodePlugin, wanted);
      done.push(`plugin   ${posix(opencodePlugin)} updated`);
    } else done.push('plugin   kaiprompt-usage.js already there (left alone — yours wins)');
  } else {
    fs.writeFileSync(opencodePlugin, opencodePluginBody(claudeUsageRoot));
...
export function uninstall({
  root, claudeDir = claudeHome(), opencodeDir = opencodeHome(),
  claudeUsageRoot = defaultClaudeUsageRoot(root),
} = {}) {
  const done = [];
  const cmds = commandsDir(claudeDir);
  ...
  if (fs.existsSync(opencodePlugin)) {
    ...
    if (body === opencodePluginBody(claudeUsageRoot) || isKaipromptPluginLoader(body)) {
