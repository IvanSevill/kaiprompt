...
export function commandFiles(root) {
  ...
  const cli = posix(path.join(root, 'promptheus.mjs'));

  return {
    'programar.md': `---
description: Programa un lanzamiento del promptheus (lo procesa el hook, 0 tokens)
...
probablemente significa que \`promptheus run\` no estaba activo a la hora programada y
...
export function shellSnippets(root) {
  const cli = posix(path.join(root, 'promptheus.mjs'));
  return {
    powershell: `function promptheus { node "${cli}" @args }`,
    bash: `alias promptheus='node "${cli}"'`,
  };
}
