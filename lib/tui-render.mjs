...
import { ago, fmt } from '../src/core/time.mjs';
import { jobDetails } from '../src/core/jobs.mjs';
import { jobPreview, resolvePrompt } from '../src/core/prompt.mjs';
import { box, c, fit, size, trunc, wrap } from './ui.mjs';
import { ICON, rows, selected, STEPS, TITLES, usageReport, VIEWS, visibleWizardSteps, wizardChoiceState } from './tui-state.mjs';
import { ACTIONS, actionDescription, actionEnabled, actionKey } from './tui-actions.mjs';
...
function usageRows(state, cols) {
  ...
  if (!report) return [c.accent(selector), '', c.muted('  Loading usage…')];
...
function wizardLines(state, cols) {
  ...
  const body = steps.flatMap((s, i) => {
    ...
    const choice = wizardChoiceState(s, wiz.values, wiz.buffer, state.catalog);
    if (s.choices || (choice.dynamic && choice.all.length)) {
    ...
    if (choice.dynamic && wiz.values.engine === 'opencode') {
      const status = state.catalog?.status;
      const note = status === 'loading' || status === 'idle'
        ? 'discovering OpenCode models…'
        : status === 'error'
          ? `model discovery failed: ${state.catalog.error}`
          : status === 'empty'
            ? 'OpenCode returned no models; type the value manually'
            : 'no matching model; type the value manually';
      return [label + (wiz.buffer ? c.accent(wiz.buffer) : c.muted('(type a value)')) + c.accent('▏'), c.muted(`             ${note}`)];
    }
...
function detailLines(detail, cols, rowsN) {
  ...
  return [...box(shown, { title: ['kaip', 'job'].join(' '), cols: Math.min(cols - 6, 76) }), indicator];
}

function helpLines(state, cols) {
  const keyWidth = cols >= 90 ? 20 : 18;
  const out = [c.bold(`  Help · opened from ${TITLES[state.helpFrom] ?? TITLES.queue}`), c.muted('  Every global action is listed. Some actions only apply when a suitable job is selected.'), ''];
  let section = null;
  for (const item of ACTIONS) {
    ...
    const description = actionDescription(item, state);
    const key = actionEnabled(item, state) ? c.accent(item.keys) : c.muted(item.keys);
    if (cols < 70) out.push('  ' + key, ...wrap(description, Math.max(10, cols - 6)).map((line) => '    ' + c.muted(line)));
    else out.push('  ' + (actionEnabled(item, state) ? c.accent(item.keys.padEnd(keyWidth)) : c.muted(item.keys.padEnd(keyWidth))) + c.muted(description));
  }
  const scheduling = 'With a time: the daemon launches it. Without a time: it waits for r. Adding never sends.';
  out.push('', c.bold('  Scheduling'), ...wrap(scheduling, Math.max(10, cols - 4)).map((line) => '  ' + c.muted(line)));
  return out;
}
...
export function render(state, dims = size()) {
  ...
  const head = [
    ...
    ...(actionEnabled(ACTIONS.find((item) => item.id === 'update'), state) ? [c.warn(`  Update v${state.update.latest} available`) + c.muted(` · ${actionKey('update')} open release`)] : []),
  ];
  ...
  else body = helpLines(state, cols);
  ...
  if (!state.wizard && !state.detail && body.length > room) {
    if (state.view === 'usage') {
      ...
      const contentRoom = Math.max(1, room - 1);
      const total = body.length;
      const max = Math.max(0, total - contentRoom);
      const start = Math.min(state.helpScroll, max);
      const shown = body.slice(start, start + contentRoom);
      body = max
        ? [...shown, c.muted(`  Help ${start + 1}-${start + shown.length} of ${total} · ↑↓ scroll`)]
        : shown;
    } else {
...
function footerKeys(state, cols) {
  const job = selected(state);
  if (cols < 70) return state.view === 'help'
    ? '↑↓ scroll · R refresh · ? help · q quit'
    : '↑↓ move · a add · R refresh · ? help · q quit';
  const key = (id, label) => `${actionKey(id)} ${label}`;
  const basic = `↑↓ · ${key('select', 'select')} · ${key('engine', 'change engine')} · ${key('add', 'add')} · ${key('edit', 'edit')} · i info · ${key('daemon', 'daemon')} · ${key('run', 'run')}`;
  ...
  return [basic, finished, retry, resume, `u usage · ${actionKey('refresh')} refresh · ${actionKey('repaint')} repaint · ${actionKey('update')} update · ${actionKey('help')} help · ${actionKey('quit')} quit`].filter(Boolean).join('  │  ');
}
