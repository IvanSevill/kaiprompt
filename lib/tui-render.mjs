...
import { ICON, rows, selected, STEPS, TITLES, usageReport, VIEWS, wizardChoices } from './tui-state.mjs';
...
function usageValue(value) {
  return value ? `${value.value}${value.partial ? ' (partial)' : ''}` : 'unavailable';
}
...
function usageRows(state, cols) {
  ...
  const selector = `  scope: ${scope.label}   ←/→ switch`;
  if (!report.sessions.length) return [c.accent(selector), '', c.muted('  (no usage data for this scope)')];

  const out = [c.accent(selector), ''];
  for (const row of report.sessions) {
    const session = row.session ?? `unknown session (${row.jobId})`;
    const where = row.target ? ` · target ${row.target}` : '';
    out.push(rowLine(`${session}${where}`, false, cols));
    out.push('    ' + fit(`input ${usageValue(row.usage.input)} · output ${usageValue(row.usage.output)} · total ${usageValue(row.usage.total)}`, cols - 4));
    if (row.usage.cost) out.push('    ' + fit(`cost $${usageValue(row.usage.cost)}`, cols - 4));
  }
  out.push('', c.bold('  totals'));
  out.push('    ' + fit(`input ${usageValue(report.totals.input)} · output ${usageValue(report.totals.output)} · total ${usageValue(report.totals.total)}`, cols - 4));
  if (report.totals.cost) out.push('    ' + fit(`cost $${usageValue(report.totals.cost)}`, cols - 4));
  return out;
}
...
const HELP_ROWS = [
  ['← → / tab / 1-5', 'switch view (← → changes the usage scope there)'],
  ['u', 'usage view; choose Claude, Codex, or an OpenCode provider'],
...
function wizardLines(state, cols) {
  ...
  const body = steps.flatMap((s, i) => {
    ...
    const choices = dynamicChoice && wiz.buffer
      ? allChoices.filter((choice) => choice.toLowerCase().includes(wiz.buffer.toLowerCase()))
      : allChoices;
