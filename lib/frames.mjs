...
export function quotaWaitFrame(job, until, pending, startedAt, view = {}, kind = null) {
  ...
  return [c.warn(fit(`  ⏸ ${label} agotado · reanudando automáticamente, mismo orden`, cols)), ...frame.slice(1)];
}
...
export function runningFrame(job, lines, startedAt, tick, { scroll = 0, expanded = false, pending = [] } = {}) {
  ...
  const head = [...quotaLines(cols, job).map((l) => '  ' + l), '', ...jobCard(job, `running · ${job.id}`, cols, { expanded })];
