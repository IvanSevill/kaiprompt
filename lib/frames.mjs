...
import { nowMs } from '../src/storage/repositories.mjs';
import { fmtTime, hhmmss, humanDur } from '../src/core/time.mjs';
import { readUsage } from '../src/adapters/claude-quota.mjs';
import { isLinked, jobPreview, resolvePrompt } from '../src/core/prompt.mjs';
 ...
 */
export function runningFrame(job, lines, startedAt, tick, { scroll = 0, expanded = false, info = false, infoScroll = 0, showDiff = false, todos = null, pending = [] } = {}) {
  const { cols, rows } = size();
  ...
  const jobTitle = c.accent('●') + ` ${['kaip', 'job'].join(' ')}`;
  const head = info
    ? [...quotaLines(cols, job).map((l) => '  ' + l), '', ...box(shownInfo, { title: jobTitle, cols: Math.min(cols - 6, 76) }), c.muted(`${Math.min(infoScroll, maxInfoScroll) + 1}/${infoLines.length} lines · ↑↓ scroll · i close`)]
    : [...quotaLines(cols, job).map((l) => '  ' + l), '', ...jobCard(job, jobTitle, cols, { expanded })];
  const foot = pendingPanel(pending, cols, 3);

  const todo = todoPanel(todos, Math.min(36, Math.max(26, Math.floor(cols / 3))));
  const sideBySide = todo.length > 0 && cols >= 96;
  const stackedTodo = todo.length > 0 && !sideBySide ? ['', ...todo] : [];
  const room = Math.max(3, rows - head.length - foot.length - stackedTodo.length - 4);
  ...
  const content = sideBySide ? columns(feed, todo, cols, room) : [...stackedTodo, ...feed];
  ...
  return [
    ...head, '',
    ...content,
    ...Array(Math.max(0, room + stackedTodo.length - content.length)).fill(''),
    status + c.muted(`  ·  ↑↓: scroll · i: kaip job · d: diff (${showDiff ? 'on' : 'off'})${showDiff && !lines.some((line) => typeof line !== 'string' && line.diff) ? ' · no changes captured yet' : ''} · Ctrl+C: stop`),
    '',
    ...foot,
  ].map((l) => fit(l, cols));
}

function todoPanel(todos, cols) {
  if (!Array.isArray(todos) || !todos.length) return [];
  const icons = { completed: '✓', in_progress: '▶', pending: '·' };
  const rows = todos.flatMap((todo) => {
    const colour = todo.status === 'completed' ? c.muted : todo.status === 'in_progress' ? c.accent : c.dim;
    const text = `${icons[todo.status] ?? '·'} ${todo.activeForm || todo.content || ''}`;
    return wrap(text, Math.max(10, cols - 4)).map(colour);
  });
  return box(rows, { title: 'TODO', cols });
}

function columns(left, right, cols, rows) {
  const gap = 2;
  const rightWidth = Math.min(cols - 24, Math.max(...right.map(width), 0));
  const leftWidth = Math.max(20, cols - rightWidth - gap);
  const out = [];
  for (let i = 0; i < rows; i++) {
    const l = fit(left[i] ?? '', leftWidth);
    const r = fit(right[i] ?? '', rightWidth);
    out.push(l + ' '.repeat(Math.max(gap, leftWidth - width(l) + gap)) + r);
  }
  return out;
}
