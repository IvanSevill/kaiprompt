...
const CLI = path.join(ROOT, 'program-prompt.mjs');
export const TASK_NAME = 'program-prompt-daemon';       // Windows Task Scheduler entry
...
export function start({ seq = false } = {}) {
  ...
  if (process.env.PROGRAM_PROMPT_NO_DAEMON) {
    return { started: false, reason: 'disabled by PROGRAM_PROMPT_NO_DAEMON', ...status() };
  }
...
export function statusLine(st = status()) {
  if (!st.running) {
    return `daemon: off — scheduled launches will NOT fire (start it with: program-prompt daemon start)`;
  }
