...
function canonicalDiffLines(name, input, cols, max = 8) {
  return normalizeToolDiffs(name, input).flatMap((diff) => {
    const rows = diff.lines.slice(0, max).map((line) => {
      const shown = `    ${trunc(line, cols - 6)}`;
      if (line.startsWith('+')) return c.ok(shown);
      if (line.startsWith('-')) return c.err(shown);
      return c.muted(shown);
    });
    if (diff.lines.length > max) rows.push(c.muted(`      … +${diff.lines.length - max} lines`));
    return rows;
  });
}
...
export function toolLines(name, input, cols = 80) {
  ...
  if (['Edit', 'MultiEdit', 'Write', 'Patch', 'ApplyPatch'].includes(n)) {
    const diffs = normalizeToolDiffs(n, input);
    const summary = n === 'Write' && diffs[0]
      ? [c.muted(`    + ${diffs[0].added} line${diffs[0].added === 1 ? '' : 's'}`)]
      : [];
    return [head, ...summary, ...canonicalDiffLines(n, input, cols)];
  }
  return [head];
}
...
/**
 * The alternate screen, entered and left AT MOST ONCE.
 *
 * The guard is the whole point. `?1049l` does not mean "clear" — it means "restore the
 * screen you saved". Called a second time, after we have already left and printed our
 * goodbye, the terminal dutifully restores the old buffer and puts the mess back: the
 * screen visibly flashed clean and then dirtied itself again. Leaving twice has to be a
 * no-op, and the only way to know is to remember.
 */
let inAlt = false;
export const altEnter = () => { if (!inAlt) { inAlt = true; w('\x1b[?1049h\x1b[?25l'); } };
export const altExit = () => { if (inAlt) { inAlt = false; w('\x1b[?1049l\x1b[?25h'); } };
/**
 * Wipe the screen — scrollback included.
 *
 * `2J` only erases what is visible; `3J` is the one that takes the scrollback with it.
 * Used between frames, so it has to stay cheap: just bytes.
 */
export const clear = () => w('\x1b[3J\x1b[2J\x1b[H');

/**
 * Wipe the screen for real, on the way out.
 *
 * Classic Windows PowerShell (conhost) does not always honour the ANSI erase codes, so the
 * frame we just tore down stays on screen and the goodbye lands on top of the debris. `cls`
 * is what actually works there — it is what you would type yourself.
 *
 * It spawns a process, so it is only for leaving. Doing this per frame would be a disaster.
 */
export function hardClear() {
  ...
  if (process.platform === 'win32') {
    try {
      ...
      return;
    } catch { /* fall through to the ANSI way */ }
  }
  clear();
}
...
// --- the window title --------------------------------------------------------
/**
 * The terminal is called "node". It should be called what it is DOING.
 *
 * With the window minimised, the taskbar is the only thing you can see — so that is where
 * the countdown belongs. OSC 0 sets it (`\x1b]0;text\x07`); `22;0t` / `23;0t` push and pop
 * the old one, which is how we give the terminal its name back on the way out. A terminal
 * left called "kaip" forever is exactly the litter this tool does not leave.
 *
 * Not a TTY → not a title. The unattended 3am batch writes nothing but its log.
 */
let titled = false;

/** Strip anything that could close the escape sequence early or paint colour into a title. */
export const titleText = (s) => strip(String(s ?? '')).replace(/[\x00-\x1f\x7f]/g, ' ').replace(/\s+/g, ' ').trim();

export function setTitle(text) {
  if (!isTTY()) return;
  const clean = titleText(text);
  if (!clean) return;
  if (!titled) { titled = true; w('\x1b[22;0t'); }      // remember the name it had
  w(`\x1b]0;${clean}\x07`);
}

/**
 * Give the name back. The empty title goes first as a fallback: if the terminal does not
 * do push/pop, it at least drops back to its default name instead of keeping ours.
 */
export function restoreTitle() {
  if (!titled) return;
  titled = false;
  w('\x1b]0;\x07\x1b[23;0t');
}
...
export function installCleanup(extra) {
  ...
  for (const sig of ['SIGINT', 'SIGTERM', 'SIGHUP']) {
    process.on(sig, () => {
      restore();
      // `process.exit()` immediately after a Ctrl+C can discard the farewell write on
      // Windows terminals. Let stdout flush after restoring the real screen first.
