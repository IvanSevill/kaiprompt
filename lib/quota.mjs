...
export function quotaVerdict(text, { now = Date.now(), usageFile, graceMs = 60_000 } = {}) {
  ...
  // The CLI error names the last limit it touched, not necessarily the limit that still
  // blocks the next launch. Prefer active windows from usage.json and wake at the nearest.
  const usage = usageFile === undefined ? readUsage() : readUsage(usageFile);
  const active = [
    ['session', usage?.session], ['weekly', usage?.weekly],
  ].flatMap(([windowKind, window]) => window && window.freePct === 0 && window.resetsAt > now
