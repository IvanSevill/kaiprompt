...
export function render(state, dims = size()) {
  ...
  // Once a launch has finished it is just history clogging the list, and the way to get rid
  // of the lot is one key — so say so ON the queue, where the mess is, instead of burying it
  // among nine other shortcuts in the footer.
  const spent = state.data.queue.filter((j) => j.status !== 'pending' && j.status !== 'running');
  const sweep = (state.view === 'queue' && spent.length && !state.wizard && !state.detail)
    ? [c.muted('  ') + c.accent('x') + c.muted(` — borrar los ${spent.length} terminados`)]
    : [];
