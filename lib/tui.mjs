// The guided GUI: `promptheus` with no arguments.
//
...
import {
  altEnter, altExit, box, c, fit, installCleanup, isTTY, paint, size, trunc, writeLines,
} from './ui.mjs';
...
export function applyEffect(effect) {
  try {
    ...
    if (effect.type === 'add') {
      ...
      const head = c.ok(`+ ${job.id}`) + ` ${preview(job.prompt, 30)}`;
...
function projectRows(state, cols) {
  ...
  if (!list.length) out.push(c.muted('  (no aliases — promptheus projects <alias> <path>)'));
...
export function render(state, dims = size()) {
  ...
  const head = [
    c.bold(c.accent('promptheus')) + c.muted('  ·  guided mode'),
    tabs,
    daemonLine(state),
    '',
  ];
  ...
  const foot = state.confirm
    ...
    : c.muted('  ↑↓ move · enter detail · a add · e edit · d del · D daemon · r run now · o out · c chat · ? help · q quit');
