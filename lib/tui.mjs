...
import { loadProjects, loadQueue, loadSessions, outPath, preview } from './store.mjs';
import { ago, fmt, parseWhen } from './time.mjs';
import { dismiss, findCutShort, resumable, resumeCutShort } from './cutshort.mjs';
...
export function applyEffect(effect) {
  try {
    ...
    if (effect.type === 'resume-cut') {
      ...
      const head = c.ok(`+ ${job.id}`) + c.muted(` continúa ${effect.hit.sessionId.slice(0, 8)}… · `)
        + c.bold('la primera');
      ...
      const d = daemon.ensure();
      if (d.reason === daemon.RUN_IS_DRAINING) return head + c.muted(` · la lanza el "run" en curso (pid ${d.runner.pid})`);
      if (d.started || d.running) return head + c.muted(` · en cuanto haya cupo (daemon pid ${d.pid})`);
      return head + c.err(' · nada procesa la cola: NO se lanzará') + c.muted(' — pulsa "D"');
    }
    ...
    if (effect.type === 'dismiss-cut') {
      ...
      return c.muted(`vale — no vuelvo a preguntar por ${effect.hit.sessionId.slice(0, 8)}…`);
    }
...
function offerLines(state, cols) {
  ...
  const body = [c.bold('Parece que una conversación se quedó a medias.'), ''];
  hits.forEach((h, i) => {
    const here = i === sel;
    ...
    body.push('    ' + c.muted(h.ask ? `«${trunc(h.ask, inner - 10)}»` : '(sin texto)'));
  });
  body.push('');
  body.push(hits.length > 1
    ? '¿Termino la marcada en cuanto vuelva el cupo?'
    : '¿La termino en cuanto vuelva el cupo?');

  return box(body, { title: 'a medias', cols: inner });
}
...
export function render(state, dims = size()) {
  ...
  const foot = state.offer
    ? '  ' + c.bold(c.accent('[enter]')) + ' sí' + c.muted('   ·   ')
      + c.bold('[esc]') + ' no' + c.muted(' (no vuelvo a preguntar)')
      + (state.offer.hits.length > 1 ? c.muted('   ·   ↑↓ elegir') : '')
    : state.confirm
