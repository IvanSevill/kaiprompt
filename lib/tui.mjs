...
import { loadLaunchDefaults, loadQueue, saveLaunchDefaults } from '../src/storage/repositories.mjs';
import { outPath } from '../src/storage/paths.mjs';
import { fmt, parseWhen } from '../src/core/time.mjs';
import { dismiss, resumeCutShort } from './cutshort.mjs';
import { addJob, clearFinished, removeJobs, retryJob } from '../src/core/jobs.mjs';
import { editJob } from '../src/core/edit.mjs';
import { renderChat, resumeCommand, resumeSpec, resumeTarget } from './chat.mjs';
...
import * as daemon from '../src/runner/daemon.mjs';
import {
...
import { ENGINE_STEPS, initialState, loadUsageData, openOffer, refresh, rows, selected, STEPS, VIEWS, visibleWizardSteps, wizardChoiceState } from './tui-state.mjs';
import { detailMaxScroll, render } from './tui-render.mjs';
import { modelCatalog } from '../src/adapters/model-catalog.mjs';
import { openRelease } from './update.mjs';
import { actionMatches } from './tui-actions.mjs';
...
function reduceWizard(state, key) {
  ...
  const choice = wizardChoiceState(step, wiz.values, wiz.buffer, state.catalog);
  if (choice.selectable) {
    const { choices, current } = choice;
...
export function reduce(state, key) {
  ...
  if (actionMatches('quit', key, state) && key === 'ctrl-c') return { state, effect: { type: 'quit' } };
  if (actionMatches('repaint', key, state)) return { state, effect: { type: 'repaint' } };

  // Printable keys belong to the active field. In particular, an uppercase R in a prompt,
  // provider, or model is text and must never refresh behind the writer's back.
  if (state.wizard) {
    const step = visibleWizardSteps(state.wizard)[state.wizard.step];
    if (actionMatches('refresh', key, state) && step.choices) return { state, effect: { type: 'refresh' } };
    return reduceWizard(state, key);
  }

  if (actionMatches('refresh', key, state)) return { state, effect: { type: 'refresh' } };
  if (actionMatches('update', key, state)) return { state, effect: { type: 'open-update', url: state.update.url } };
  ...
  if (state.offer) {
    ...
    if (actionMatches('quit', key, state)) return { state, effect: { type: 'quit' } };
  ...
  const view = (v) => ({
    state: {
      ...state, view: v, sel: 0, helpScroll: v === 'help' ? 0 : state.helpScroll,
      helpFrom: v === 'help' && state.view !== 'help' ? state.view : state.helpFrom, message: null,
    },
    effect: v === 'usage' && state.data.usageStatus === 'idle' ? { type: 'load-usage' } : null,
  });

  if (actionMatches('quit', key, state)) return { state, effect: { type: 'quit' } };
  if (actionMatches('help', key, state)) return view('help');
  if (key === 'u') return view('usage');
  if (key >= '1' && key <= String(VIEWS.length)) return view(VIEWS[Number(key) - 1]);
  if (state.view === 'help' && (key === 'up' || key === 'down')) {
    return { state: { ...state, helpScroll: Math.max(0, state.helpScroll + (key === 'down' ? 1 : -1)) }, effect: null };
  }
  ...
  // `y` walks INTO the conversation: hands the terminal to the matching interactive CLI
  // resumed on that session. Reading a transcript ("c") tells you what happened;
...
export function readOutput(id) {
  ...
  let resume = '';
  try { if (job.sessionId) resume = c.muted(`\nresume: ${resumeCommand(job)}`); } catch { /* unsupported adapter */ }
...
export async function startTUI() {
  ...
  const catalogLoad = modelCatalog.load();
  let state = refresh(initialState({ offer: openOffer(), catalog: modelCatalog.snapshot() }));
  ...
  catalogLoad.then((catalog) => {
    if (!quitting) { state = { ...state, catalog }; draw(); }
  });
  ...
  async function handle(effect) {
    switch (effect.type) {
      ...
      case 'refresh': {
        const catalog = modelCatalog.reload();
        state = refresh({ ...state, message: c.muted('refreshing data and models…') });
        if (state.view === 'usage' || state.data.usageStatus === 'ready') state = loadUsageData(state);
        state = { ...state, catalog: await catalog, message: c.ok('data and models refreshed') };
        return;
      }

      case 'load-usage':
        state = loadUsageData(state);
        return;

      case 'open-update':
        try {
          const url = openRelease(effect.url);
          state = { ...state, message: c.ok('opened release: ') + c.muted(url) };
        } catch (e) {
          state = { ...state, message: c.err(e.message) };
        }
        return;

      case 'resume': {                   // walk into the matching interactive engine
        let where;
        ...
        // stdio: 'inherit' — the engine takes the terminal whole, keyboard included.
        // We are just the doorway; we get the terminal back when it exits.
        await new Promise((resolve) => {
          const spec = resumeSpec(where);
          const child = spawn(spec.command, spec.args, {
            cwd: spec.cwd, stdio: 'inherit', shell: process.platform === 'win32',
          });
          child.on('exit', resolve);
          child.on('error', (e) => {
            state = { ...state, message: c.err(`could not start ${spec.command}: ${e.message}`) };
            resolve();
          });
        });
