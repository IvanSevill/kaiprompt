...
import {
  altEnter, altExit, box, c, clear, fit, installCleanup, isTTY, paint, size, trunc, writeLines,
} from './ui.mjs';
...
export function reduce(state, key) {
  ...
  if (key === 'x') {
    ...
    return {
      state: {
        ...state,
        confirm: { text: `clear ${spent.length} finished job(s)?`, effect: { type: 'clear' } },
      },
      effect: null,
    };
  }
  ...
  switch (key) {
    ...
    case 'd':
      return {
        state: {
          ...state,
          confirm: { text: `delete ${item.id} (${jobPreview(item, 30)})?`, effect: { type: 'delete', id: item.id } },
        },
        effect: null,
      };
    case 'o':
...
export function applyEffect(effect) {
  try {
    ...
    if (effect.type === 'add') {
      ...
      // Queued, not launched. If it has a time, the daemon has to be up to keep that
      // promise — so bring it up here. If it doesn't, say plainly that nothing will run.
      if (!job.when) return head + c.muted('  · sequential: only runs when you press "r"');
      const d = daemon.ensure();
      return head + c.muted(`  · ${fmt(job.when)} · `)
        + (d.started ? c.ok(`daemon started (pid ${d.pid})`) : c.ok('daemon on')) ;
    }
    if (effect.type === 'edit') {
      const v = effect.values;
...
const HELP_ROWS = [
  ...
  ['x', 'clear every finished job (asks first)'],
...
/** The daemon line under the tabs: the answer to "will this actually fire?". */
function daemonLine(state) {
  const d = state.data.daemon;
  if (!d?.running) {
    return c.warn('◇ daemon off') + c.muted(' — scheduled launches will NOT fire · "D" to arm it');
  }
  const next = d.next ? c.muted(' · next ') + fmt(d.next) : c.muted(' · nothing scheduled');
  return c.ok('◆ daemon on') + c.muted(` (pid ${d.pid})`) + next;
}
...
export function render(state, dims = size()) {
  ...
  const foot = state.confirm
    ...
    : c.muted('  ↑↓ · a add · e edit · d del · x clear · D daemon · r run  │  '
...
/**
 * The goodbye. A clean screen, and the one thing you actually want to know as you walk
 * away: is anything still going to happen, and will anything be there to make it happen?
 * Closing the GUI on a queue full of scheduled work with the daemon off is the exact
 * mistake this tool exists to prevent — so say so on the way out.
 */
function farewell() {
  ...
  if (!pending.length) {
    ...
    if (scheduled.length && !d.running) {
      out.push('', c.err('  ⚠ el daemon está apagado: lo agendado NO se va a lanzar.'),
    ...
    if (pending.length > scheduled.length) {
      out.push(c.muted(`  ${pending.length - scheduled.length} sin hora: esperan a un `)
