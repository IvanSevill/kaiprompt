...
import { loadQueue, outPath, preview } from './store.mjs';
import { fmt, parseWhen } from './time.mjs';
import { dismiss, resumeCutShort } from './cutshort.mjs';
import { addJob, clearFinished, removeJobs } from './queue.mjs';
...
import {
  altEnter, altExit, c, clear, installCleanup, isTTY, paint, size, writeLines,
} from './ui.mjs';

import { keyReader, pasteText, PASTE_OFF, PASTE_ON } from './tui-keys.mjs';
import { ENGINE_STEPS, initialState, openOffer, PERMS, refresh, rows, selected, STEPS, VIEWS, wizardChoices } from './tui-state.mjs';
import { render } from './tui-render.mjs';
...
function reduceWizard(state, key) {
  ...
  if (choiceStep) {
    ...
    if (key === 'left' || key === 'up') {
      const v = choices[(i - 1 + choices.length) % choices.length];
      return keep({ wizard: selectedWizardValue(wiz, step, v) });
    }
    if (key === 'right' || key === 'down' || key === 'space') {
      const v = choices[(i + 1) % choices.length];
      return keep({ wizard: selectedWizardValue(wiz, step, v) });
    }
  }
  ...
  if (key === 'enter') {
    ...
    if (step.key === 'provider' && wiz.values.engine === 'opencode' && !raw) {
      return { state: { ...state, message: c.err('choose or type an OpenCode provider') }, effect: null };
    }
    if (step.key === 'model' && wiz.values.engine === 'opencode' && !raw) {
      return { state: { ...state, message: c.err('choose or type an OpenCode model') }, effect: null };
    }
    ...
    if (wiz.step === steps.length - 1) {
      const effect = wiz.mode === 'add'
        ? { type: 'add', values }
        : wiz.mode === 'bulk-engine' ? { type: 'bulk-engine', ids: wiz.ids, values } : { type: 'edit', id: wiz.id, values };
