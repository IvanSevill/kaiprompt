...
/** Rotate the token: every paired phone stops working until it pairs again. */
...
/** What goes in the QR: where to connect, and the proof you are allowed to. */
export function pairingPayload(port = DEFAULT_PORT) {
  const { token } = serverConfig();
  const best = addresses(port).find((a) => a.tailscale) || addresses(port)[0];
  return {
    v: 1,
    url: best ? best.url : `http://127.0.0.1:${port}`,
    token,
    host: os.hostname(),
    tailscale: Boolean(best?.tailscale),
  };
}
...
export function createServer({ port = DEFAULT_PORT } = {}) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (!authorized(req, conf.token)) return json(res, { error: 'unauthorized' }, 401);

    try {
      if (seg[0] !== 'api') return json(res, { error: 'not found' }, 404);

      // GET /api/state — the whole main screen in one call
      if (seg[1] === 'state' && seg.length === 2) return json(res, stateDTO());

      // GET /api/targets — conversations, grouped
      if (seg[1] === 'targets' && seg.length === 2) return json(res, targetsDTO());

      // GET /api/job/:id            → the job + its final answer
      // GET /api/job/:id/chat       → the WHOLE conversation it had
      if (seg[1] === 'job' && seg[2]) {
        if (seg[3] === 'chat') return json(res, chatDTO(seg[2]));
        return json(res, outputDTO(seg[2]));
      }

      // GET /api/chat/:ref — by target or session id
      if (seg[1] === 'chat' && seg[2]) return json(res, chatDTO(seg[2]));
