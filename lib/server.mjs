...
import { clearFinished } from '../src/core/jobs.mjs';
...
import {
  chatDTO, hideFinishedConversations, outputDTO, snapshotDTO, stateDTO, targetsDTO, usageDTO,
} from './server-dto.mjs';
import { isTerminalStatus, replayLive, subscribeLive } from '../src/events/live.mjs';
import { createQuotaLoader } from '../src/adapters/quota-client.mjs';

const defaultLoadQuota = createQuotaLoader();
...
function validCallbackUrl(value) {
  ...
  try {
    ...
    if (!['http:', 'https:'].includes(url.protocol) || !url.hostname || url.username || url.password ||
        url.pathname !== '/job-done' || url.search || url.hash ||
        (url.port && (Number(url.port) < 1 || Number(url.port) > 65535))) return undefined;
    return url.toString();
...
export {
  chatDTO, conversationStatus, hideFinishedConversations, outputDTO, snapshotDTO, stateDTO,
  targetsDTO, usageDTO,
} from './server-dto.mjs';
...
export function createServer({
  port = DEFAULT_PORT, loadChat = chatDTO, loadTargets = targetsDTO, loadQuota = defaultLoadQuota,
} = {}) {
  ...
  const server = http.createServer((req, res) => {
    ...
    try {
      ...
      // GET /api/snapshot — state and conversation visibility from one request.
      if (seg[1] === 'snapshot' && seg.length === 2) return json(res, snapshotDTO(), 200, enc);
      ...
      // GET /api/quota?provider=claude|codex — live entitlement, never historical usage.
      if (seg[1] === 'quota' && seg.length === 2 && req.method === 'GET') {
        const provider = url.searchParams.get('provider') || 'claude';
        if (!['claude', 'codex'].includes(provider)) {
          return json(res, { error: 'provider must be claude or codex' }, 400, enc);
        }
        return Promise.resolve(loadQuota(provider))
          .then((quota) => json(res, quota, 200, enc))
          .catch((error) => json(res, {
            provider, status: 'error', source: { kind: null, official: null },
            freshness: { observedAt: null, stale: null }, limits: {}, plan: null, credits: null,
            error: { code: 'collector-error', message: error?.message ?? null },
          }, 200, enc));
      }
      ...
      // GET /api/conversations/:conversationId — stable identity; old chat routes remain.
      if (seg[1] === 'conversations' && seg[2] && req.method === 'GET') {
        return json(res, loadChat(decodeURIComponent(seg[2])), 200, enc);
      }

      if (seg[1] === 'conversations' && seg[2] === 'hide-finished' && req.method === 'POST') {
        return json(res, hideFinishedConversations(), 200, enc);
      }
      ...
      if (seg[1] === 'device' && req.method === 'POST') {
        ...
        req.on('end', () => {
          ...
          const url = validCallbackUrl(dev.url);
          if (url === undefined) return json(res, { error: 'invalid callback url' }, 400, enc);
      ...
      if (seg[1] === 'device' && seg[2] && seg.length === 3 && req.method === 'DELETE') {
        ...
        // Only the last phone returning the server to pairing resets global session evidence.
        // Removing one phone must not make another currently paired phone disappear.
        if (c2.devices.length === 0) c2.pairingResetAt = Date.now();
        saveServerConfig(c2);
        publish({ type: 'devices', devices: c2.devices.map((d) => ({ name: d.name, pairedAt: d.pairedAt })) });
        return json(res, {
          ok: true, removed, registered: false, devices: c2.devices.length,
          mode: c2.devices.length ? 'connected' : 'pairing', protocol: 2,
        }, 200, enc);
      }
