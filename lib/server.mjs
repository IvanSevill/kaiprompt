...
import { replayLive, subscribeLive } from './live-events.mjs';
...
export { chatDTO, outputDTO, stateDTO, targetsDTO, usageDTO } from './server-dto.mjs';
    ...
    try {
      ...
      if (seg[1] === 'events') {
        ...
        if (jobId) {
          ...
          else replay.forEach(writeEvent);
        }
        listeners.add(writeEvent);
        const unsubscribe = subscribeLive(writeEvent);
        const beat = setInterval(() => { try { res.write(': ping\n\n'); } catch { /* gone */ } }, 20_000);
        req.on('close', () => { clearInterval(beat); listeners.delete(writeEvent); unsubscribe(); });
        return undefined;
      }
