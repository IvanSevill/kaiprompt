...
export function createServer({ port = DEFAULT_PORT } = {}) {
  ...
  const server = http.createServer((req, res) => {
    ...
    // catastrophe through the tunnel. That check is the only thing standing between this
    // page and giving your keys to anyone who guesses the URL.
    if (url.pathname === '/pair') {
      const local = ['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(req.socket.remoteAddress);
      if (!local) return json(res, { error: 'not found' }, 404);
