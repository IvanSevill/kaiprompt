...
import {
  ADAPTERS, DATA, ROOT, importProgramados, loadQueue, loadSessions, nowMs,
  outPath, patchJob, preview, saveSessions,
} from './store.mjs';
import { fmt, fmtTime, hhmmss, humanDur } from './time.mjs';
import {
  altEnter, bar, bigText, bigWidth, box, c, centerLine, installCleanup,
  isTTY, paint, size, SPINNER, toolSummary, trunc,
} from './ui.mjs';
...
function eventLines(evt, cols) {
  ...
  for (const b of blocks) {
    if (b.type === 'text' && b.text?.trim()) {
      ...
      out.push(c.accent('⏺') + ' ' + c.bold(name) + c.muted(arg ? `(${arg})` : ''));
    }
  }
  return out;
}
...
export async function runQueue({ once = false, dryRun = false } = {}) {
  ...
  try {
    return isTTY() ? await runTUI({ once }) : await runPlain({ once });
  } finally {
    release();
  }
}
