...
function idleFrame() {
  ...
  const body = [
    ...
    ...box([
      ...
      '',
      c.accent('  kaip add "…" --at +10m'),
      c.muted('      from another terminal'),
      c.accent('  /programar +10m | …'),
      c.muted('      from a Claude chat (0 tokens)'),
    ], { title: 'idle', cols: Math.min(cols - 8, 52) }).map((l) => centerLine(l, cols)),
    '',
