...
export function reapStale() {
  ...
  let n = 0;
  for (const j of q) {
    if (j.status !== 'running' || !j.runnerPid || alive(j.runnerPid)) continue;
    j.status = 'error';
    j.finishedAt = nowMs();
 ...
 */
async function runPlain({ once, scheduledOnly = false, loop = false, pollMs = 15_000 }) {
  ...
  try {
    for (;;) {
      ...
      if (!job) {                                   // only future scheduled jobs left
        ...
        repaint = () => paint(clockFrame(upcoming, next, pending, waitStart, view));
      ...
      const { cols } = size();
      const behind = pending.filter((j) => j.id !== job.id);
      const lines = [];
      let tick = 0;
 ...
 */
async function runParallel({ once, max, scheduledOnly = false, pollMs = 15_000, tty = isTTY() }) {
  ...
  try {
    for (;;) {
      ...
      const times = loadQueue()
        ...
        .filter(Boolean);
      if (!times.length) break;                     // nothing running, nothing coming
...
export async function runQueue({
  ...
  pollMs = 15_000, parallel = 1, plain = false,
} = {}) {
  ...
  try {
    if (max > 1) return await runParallel({ once, max, scheduledOnly, pollMs, tty: tui });
    return tui
      ? await runTUI({ once })
      : await runPlain({ once, loop, scheduledOnly, pollMs });
  } finally {
    release();
  }
}
