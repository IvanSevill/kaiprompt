...
import {
  altEnter, bar, bigText, bigWidth, box, c, centerLine, fit, installCleanup, width,
...
export async function executeJob(job, { dryRun = false, onEvent } = {}) {
  ...
  const prompt = resolvePrompt(job);
...
function idleFrame() {
  ...
  const guard = d.running
    ...
    : c.warn('◇ daemon off') + c.muted(' — close this window and nothing fires · ')
  ...
  const body = [
    ...box([
      ...
      '',
      guard,
    ], { title: 'idle', cols: Math.min(cols - 8, 66) }),
    '',
    quotaBar(cols),                       // centred with the box, not adrift on the left
    '',
    c.muted('  Ctrl+C to stop'),
  ];

  // Centre the whole block as one, measuring by the widest line: centring each line on
  // its own is what left the quota bar floating away from the box.
  const w = Math.max(...body.map(width));
  const left = ' '.repeat(Math.max(0, Math.floor((cols - w) / 2)));
  const top = Math.max(0, Math.floor((rows - body.length) / 2));

  return [...Array(top).fill(''), ...body.map((l) => fit(left + l, cols))];
}
...
async function runPlain({ once, scheduledOnly = false, loop = false, watch = false, pollMs = 15_000 }) {
  ...
  for (;;) {
    ...
    patchJob(job);
    log(`▶ ${job.id} [${job.adapter}${job.target ? '/' + job.target : ''}] ${jobPreview(job)}`);
