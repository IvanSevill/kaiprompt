...
export function runnerStatus() {
  ...
  if (!lock.held) {
    ...
    if (daemon.running) return { willFire: true, kind: 'daemon', pid: daemon.pid, durable: true };
    return { willFire: false, kind: null, pid: null, durable: false };
  }
...
export function runnerLine(st = runnerStatus()) {
  if (!st.willFire) {
    return { ok: false, text: 'nada está procesando la cola: lo agendado NO se lanzará', hint: 'kaip daemon start' };
  }
  if (st.kind === 'daemon') {
    return { ok: true, text: `daemon activo (pid ${st.pid}) — se lanzará solo, aunque cierres todo`, hint: null };
  }
  return {
    ok: true,
    text: `un "kaip run" está procesando la cola (pid ${st.pid})`,
    hint: 'si cierras esa ventana, deja de lanzarse: kaip daemon start',
  };
}
