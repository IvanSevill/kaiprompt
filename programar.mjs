...
const HOME = process.env.KAIP_HOME || __dir;
...
async function armDaemon() {
  if (process.env.KAIP_NO_DAEMON) return '  (daemon desactivado por entorno)';
  try {
    ...
    return `  ⚠ no pude arrancar el daemon (${e.message}); lánzalo con: kaip daemon start`;
  }
}
