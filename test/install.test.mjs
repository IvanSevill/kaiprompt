...
const ROOT = 'C:/tools/chat-queue';                          // una instalación de ejemplo
...
test('commandFiles: los dos comandos llevan la ruta REAL de instalación', () => {
  ...
  assert.match(files['programar.md'], /C:\/tools\/chat-queue\/programar\.mjs/);
  assert.match(files['resumen-prompts.md'], /C:\/tools\/chat-queue\/program-prompt\.mjs/);
  assert.match(files['resumen-prompts.md'], /C:\/tools\/chat-queue\/out/);
});
...
test('shellSnippets: el atajo lleva la ruta real, y entrecomillada', () => {
  ...
  assert.match(powershell, /function program-prompt/);
  assert.match(powershell, /"C:\/tools\/chat queue\/program-prompt\.mjs"/, 'con espacios debe ir entre comillas');
  assert.match(bash, /alias program-prompt=/);
});

test('posix: en el JSON van barras normales (una barra invertida habría que escaparla)', () => {
  assert.equal(posix('C:\\tools\\chat-queue'), 'C:/tools/chat-queue');
});
