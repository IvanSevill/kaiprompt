...
export async function executeJob(job, { dryRun = false, onEvent } = {}) {
  ...
  // A target may supply an already-known session even when this individual job did not.
  // Put it on the job before launching so an interrupted resume is still self-describing.
  if (sid && !job.sessionId) {
    job.sessionId = sid;
    patchJob(job);
  }
  ...
  let sessionPersistedFromEvent = false;
  const forwardEvent = (event) => {
    if (event?.session_id && !job.sessionId) sessionPersistedFromEvent = true;
    recordAdapterEvent(job, event);
    onEvent?.(event);
  };
  const res = await adapter.run({
    prompt, sessionId: sid, dryRun, dir: job.dir || null,
    permMode: job.permMode || null, model: job.model || null, provider: job.provider || null, onEvent: forwardEvent,
  });
...
export function requeue(job, plan) {
  ...
  patchJob(job);
  emitLive(job, { kind: 'status', status: 'pending', pausedUntil: job.pausedUntil });
  return job;
}
...
export function markRunning(job) {
  ...
  job.attemptId = newAttemptId();
  patchJob(job);
  emitLive(job, { kind: 'status', status: 'running', preview: jobPreview(job, 90) });
  return job;
}
...
export async function commit(job, end) {
  ...
  patchJob(job);
  emitLive(job, { kind: 'status', status: job.status, error: job.error ?? null });
  await announce(job);
  return job;
}
